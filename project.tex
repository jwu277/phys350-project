...
\begin{flushleft}
    ...
    Rewrite the $x$ EL equations in terms of $X$, applying the simplification that $y_{ijk}$ and $z_{ijk}$ are not displaced. Also show that $\ddot{y}_{ijk}$ and $\ddot{z}_{ijk}$ are equal to zero in the EL equations (this ensures that if the system starts with $y_{ijk}$ and $z_{ijk}$ at rest for all $(i, j, k)$, $y_{ijk}$ and $z_{ijk}$ will remain unperturbed).
